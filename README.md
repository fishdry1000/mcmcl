...
- **管理**：「编辑」修改该实例的启动设置（Java 路径、最大内存、版本隔离），「Mod 目录」直达该实例实际使用的 mods 文件夹（仅模组实例可用）。
...
开启版本隔离后，实例以 `versions/<实例 ID>/` 为运行目录，各自使用独立的 `mods`、`saves`、配置和日志；`libraries` 与 `assets` 等大文件仍在仓库中共享。实例编辑页可以选择继承全局策略，或为单个实例强制开启/关闭。切换策略不会自动搬移已有文件。
