...
模组本身不再解析 `launch.json`，也不再直接拼接 Minecraft 启动命令。对于已经准备好的 HMCL 仓库，实例 manifest 的读取/继承、启动参数生成、原生库处理和目标进程管理均由 helper/HMCL Core 负责。模组还支持在游戏内安装新的原版实例（从 Mojang 官方源或 BMCLAPI 镜像下载版本 JSON、客户端 jar、库与资源文件）以及为已有实例补齐缺失文件（修复）；账号可选择沿用当前 Minecraft 会话或使用离线账号。
...
- `install`/`repair` 只覆盖原版实例；模组加载器（Forge/Fabric 等）实例仍需由 HMCL 等外部工具安装。
